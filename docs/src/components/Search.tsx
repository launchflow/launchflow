...
function HighlightQuery({ text, query }: { text: string; query: string }) {
  return (
    <Highlighter
      highlightClassName="group-aria-selected:underline bg-transparent text-primary dark:text-primary"
      searchWords={[query]}
      autoEscape={true}
...
function SearchResult({
  ...
  let sectionTitle = homeNavigation.find((section) =>
  ...
  return (
    ...
    >
      <div
        id={`${id}-title`}
        aria-hidden="true"
        className="text-sm text-slate-700 group-aria-selected:text-primary dark:text-slate-300 dark:group-aria-selected:text-primary"
      >
...
>(function SearchInput({ autocomplete, autocompleteState, onClose }, inputRef) {
  ...
  return (
    <div className="group relative flex h-12">
      <SearchIcon className="pointer-events-none absolute left-4 top-0 h-full w-5 fill-slate-400 dark:fill-background_dark" />
      <input
        ref={inputRef}
        className={clsx(
...
function SearchDialog({
  ...
  return (
    <>
      ...
      >
        <div className="fixed inset-0 bg-background_dark/50 backdrop-blur" />

        <div className="fixed inset-0 overflow-y-auto px-4 py-4 sm:px-6 sm:py-20 md:py-32 lg:px-8 lg:py-[15vh]">
          <Dialog.Panel className="mx-auto transform-gpu overflow-hidden rounded-xl bg-white shadow-xl sm:max-w-xl dark:bg-background_dark dark:ring-1 dark:ring-slate-700">
            <div {...autocomplete.getRootProps({})}>
              ...
              >
                ...
                <div
                  ref={panelRef}
                  className="border-t border-slate-200 bg-white px-2 py-3 empty:hidden dark:border-slate-400/10 dark:bg-background_dark"
...
export function Search() {
  ...
  return (
    <>
      <button
        type="button"
        className="group flex h-6 w-6 items-center justify-center sm:justify-start md:h-auto md:w-80 md:flex-none md:rounded-lg md:py-2.5 md:pl-4 md:pr-3.5 md:text-sm md:ring-1 md:ring-slate-200 md:hover:ring-slate-300 lg:w-96 dark:md:bg-background_dark/75 dark:md:ring-inset dark:md:ring-white/5 dark:md:hover:bg-slate-700/40 dark:md:hover:ring-slate-500"
        {...buttonProps}
      >
