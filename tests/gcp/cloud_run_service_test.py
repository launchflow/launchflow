...
from launchflow.gcp.cloud_run import CloudRunService, CloudRunServiceReleaseInputs
from launchflow.gcp.cloud_run_container import CloudRunServiceContainerOutputs
...
class CloudRunServiceTest(unittest.IsolatedAsyncioTestCase):
    ...
    @mock.patch("launchflow.gcp.cloud_run.build_artifact_registry_docker_image")
    async def test_build_cloud_run(self, build_docker_mock: mock.MagicMock):
        build_docker_mock.return_value = "fake-docker-image:latest"

        cloud_run_service = CloudRunService("service_name")

        # Setup the resource output mocks
        artifact_registry_outputs = ArtifactRegistryOutputs(
            docker_repository="test-docker-repository",
        )
        cloud_run_service._artifact_registry.outputs = mock.MagicMock(
            return_value=artifact_registry_outputs
        )

        # Run the build and validate the result / mock calls
        release_inputs = await cloud_run_service._build(
    ...
    @mock.patch("launchflow.gcp.cloud_run.promote_artifact_registry_docker_image")
    async def test_promote_cloud_run_service(self, promote_docker_mock: mock.MagicMock):
        ...
        release_inputs = await cloud_run_service._promote(
    ...
    @mock.patch("launchflow.gcp.cloud_run._release_cloud_run")
    async def test_release_cloud_run_successful(
        self, release_cloud_run_mock: mock.MagicMock
    ):
        cloud_run_service = CloudRunService("service_name")

        # Setup the resource output mocks
        container_outputs = CloudRunServiceContainerOutputs(
            service_url="https://test-service-url",
        )
        container_outputs.gcp_id = "test-gcp-id"

        cloud_run_service._cloud_run_service_container.outputs = mock.MagicMock(
            return_value=container_outputs
        )

        # Run the release and validate the result
        await cloud_run_service._release(
        ...
        self.assertEqual(
            cloud_run_service.outputs().service_url,
            "https://test-service-url",
        )

        release_cloud_run_mock.assert_called_once_with(
            cloud_run_id="test-gcp-id",
            docker_image="gcr.io/test-project/test-image:foo",
            gcp_environment_config=self.dev_gcp_environment_config,
            launchflow_uri=self.dev_launchflow_uri,
            deployment_id="test-deployment-id",
    ...
    async def test_cloud_run_service_outputs(self):
        default_cloud_run_service = CloudRunService("service_name")
        custom_url_cloud_run_service = CloudRunService(
        ...
        default_cloud_run_service._cloud_run_service_container.outputs = mock.MagicMock(
            return_value=container_outputs
        )
        custom_url_cloud_run_service._cloud_run_service_container.outputs = (
            mock.MagicMock(return_value=container_outputs)
        )

        ip_address_outputs = GlobalIPAddressOutputs(
            ip_address="1.2.3.4",
        )
        custom_url_cloud_run_service._ip_address.outputs = mock.MagicMock(  # type: ignore
        ...
        custom_url_cloud_run_service._ssl_certificate.outputs = mock.MagicMock(  # type: ignore
            return_value=ssl_outputs
        )

        # Fetch the outputs and assert that they are correct
        default_service_outputs = default_cloud_run_service.outputs()
        self.assertEqual(
        ...
        custom_url_service_outputs = custom_url_cloud_run_service.outputs()
        self.assertEqual(
