...
class EnvironmentsIntegrationTest(unittest.TestCase):
    ...
    def test_gcp_environment_flows(self):
        ...
        for env in environments:
            # TODO: Run both commands concurrently
        ...
        list_output = subprocess.check_output(
            "lf environments list --format=yaml", shell=True, cwd=self.working_dir
        ).decode("utf-8")
        ...
        with self.assertRaises(subprocess.CalledProcessError) as context:
            subprocess.run(
                f"lf environments delete {environment_a} -y",
                shell=True,
                cwd=self.working_dir,
                capture_output=True,
                text=True,
        ...
        list_output = subprocess.check_output(
            "lf environments list", shell=True, cwd=self.working_dir
        ).decode("utf-8")
        ...
        result = subprocess.check_call(
            f"lf destroy {environment_b} --service fastapi-service -y",
            shell=True,
            cwd=self.working_dir,
        )
        self.assertEqual(result, 0)
        ...
        with self.assertRaises(subprocess.CalledProcessError) as context:
            subprocess.run(
                f"lf environments delete {environment_b} -y",
                shell=True,
                cwd=self.working_dir,
                capture_output=True,
                text=True,
